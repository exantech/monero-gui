...
# Copy dependencies
EXCLUDE='libstdc++|libgcc_s.so|libc.so|libpthread|libdl|libGL|libm|libz|libglib|libgthread|librt'
INCLUDE='libunbound'
...
# cp -v $QTXML_DIR/libQt5QuickTemplates2.so.5 $TARGET/libs || exit
cp -v $QTXML_DIR/libQt5*.so.5 $TARGET/libs || exit
# Create start script
